...
_.assign(Orderbook.prototype, new function() {
  ...
  prototype.edge = function(side) {
    if(side == 'buy') {
      return parseFloat(this._bids.max().price.toString());
    }
    return parseFloat(this._asks.min().price.toString());
  }

  prototype.ask = function() {
    return this.edge('sell');
  }

  prototype.bid = function() {
    return this.edge('buy');
  }
